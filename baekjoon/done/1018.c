...
int countWrong(Map *map, Pos startPos, char startChar)
{
    ...
    for (int row = 0; row < 8; row++)
    {
        ...
        {
            Pos pos = addPos(startPos, (Pos){row, column});
            if (current != *getElement(map, pos))
                count++;
...
int main()
{
    ...
    int rowIter = map->row - 7;
    int columnIter = map->column - 7;

    int min = 64;

    for (int row = 0; row < rowIter; row++)
    {
        for (int column = 0; column < columnIter; column++)
        {
            Pos startPos = {row, column};
            int count = countWrong(map, startPos, 'W');
            if (count < min)
                min = count;
            count = countWrong(map, startPos, 'B');
            if (count < min)
                min = count;
        }
    }
